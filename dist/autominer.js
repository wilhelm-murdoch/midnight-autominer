...
async function isFindingASolution() {
    const solutionElement = Array.from(document.querySelectorAll("*")).find((element) => {
        const textNodes = Array.from(element.childNodes).filter((node) => node.nodeType === Node.TEXT_NODE);
        return textNodes.some((node) => node.textContent?.includes("Finding a solution..."));
    });
...
async function handleMinerIntervention(isInitial = true) {
    if (isInitial) {
        console.log(`⚠️ [Fresh Tab Reload ${new Date().toISOString()}] Miner is inactive and not actively crunching numbers. Attempting to start the session...`);
        startMiningSession();
    }
    else {
        console.log(`⚠️ [Polling cycle ${new Date().toISOString()}] Miner is inactive and not actively crunching numbers. Attempting to reload the tab and start the session...`);
        window.location.reload();
    }
}
async function startAutoResume(intervalMs = 5000) {
